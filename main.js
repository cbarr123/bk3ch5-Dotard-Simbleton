...
const createHTMLRep = (busObject) => {
    return `
        <section>
            <h2>${busObject.companyName}</h2>
            <h4>${busObject.companyIndustry}</h4>
            <p>${busObject.addressFullStreet}</p>
            <p>${busObject.addressCity}, ${busObject.addressStateCode}  ${busObject.addressZipCode}</p>
        </section>
    `
}
...
businesses.filter(busObject => {
    if (busObject.addressStateCode === "NY"){
        const htmlRepresentation = createHTMLRep(busObject)
        createPlaceForHTMLRep.innerHTML += htmlRepresentation
        }
   })
